...
export default function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchWatchlistStatus = async () => {
      ...
      const userId = user?.sub?.substring(14)
      try {
        const response = await fetch(`${API_URL}/api/startup/get-watchlist-status/${params.id}`, {
          method: "GET",
          headers: {
            user_id: userId,
          },
  ...
  useEffect(() => {
    const fetchUpvoteStatus = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/startup/get-upvote-status/${params.id}`, {
          method: "POST",
          headers: {
            user_id: userId,
          },
  ...
  useEffect(() => {
    const fetchUpvoteCounts = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/startup/get-upvote-count/${params.id}`, {
          method: "GET",
          headers: {
            user_id: userId,
          },
  ...
  const handleDeleteMember = async (memberId) => {
    try {
      ...
      const response = await axios.delete(`${API_URL}/api/startup/delete-teamMember`, {
        headers: {
          "Content-Type": "application/json",
          user_id: userId,
        },
  ...
  const handleUpvoteStatus = async () => {
    ...
    try {
      const userId = user.sub?.substring(14)
      const response = await fetch(`${API_URL}/api/startup/upvote-startup/${params.id}`, {
        method: "POST",
        headers: {
          user_id: userId,
        },
      ...
      if (response.status === 200) {
        const statusResponse = await fetch(`${API_URL}/api/startup/get-upvote-status/${params.id}`, {
          method: "POST",
          headers: {
            user_id: userId,
          },
      ...
      if (response.status === 200) {
        const countResponse = await fetch(`${API_URL}/api/startup/get-upvote-count/${params.id}`, {
          method: "GET",
          headers: {
            user_id: userId,
          },
  ...
  const handleLikeUpdate = async (updateId: string) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/profile/like-update`,
        { update_id: updateId },
        {
          headers: {
            user_id: userId,
      ...
      // After liking, fetch the updated updates data
      if (response.status === 200 || response.status === 201) {
        // Fetch updates again to get the updated like counts
          ...
          <div className="absolute z-20 bottom-0 left-0 p-6 flex items-end gap-4">
            ...
            <div>
              ...
              <p className="text-gray-300 mt-1">
                {" "}
                Founder:
                <a href={`/public-profile/founder/${project.id}`} className="text-blue-600">
                  {" "}
                  {project.founderName}
                </a>
              </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <Button
                          ...
                          className="w-full justify-start text-gray-300 border-gray-700"
                        >
                          <a href={project.pitchDeck} target="_blank" rel="noopener noreferrer">
                      ...
                      {updates.map((update, index) => (
                        <div key={index} className="space-y-4">
                          ...
                          <div className="flex gap-4 mt-2">
                            ...
                            >
                              <ThumbsUp className="w-4 h-4" fill={hasLiked ? "white" : ""} />
                              {upvotingUpdate ? "..." : <span> {update.likes} </span>}
                            </button>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs">
                      <span className="text-gray-400">{project.progress > 100 ? 100 : project.progress}% funded</span>
                      <span className="text-amber-400">{daysLeft < 0 ? 0 : daysLeft} days left</span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-white">{daysLeft < 0 ? 0 : daysLeft}</div>
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <p className="text-gray-300 text-sm mt-1">
                          OnlyFounders has analyzed this project and assigned it a low risk score based on team
                          credentials, code quality, and tokenomics model.
                        </p>
                      </div>
                    </div>
                  </div>
